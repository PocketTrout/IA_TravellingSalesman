        ...
        if len(self._cities) > 2:
            ...
            for city in self._cities:
                ...
                for neighbours in self._cities:
                    distance = math.sqrt((neighbours.coords().x() - city.coords().x()) ** 2 + (neighbours.coords().y() - city.coords().y()) ** 2)
    ...
    def fillPartByCrossing(self, firstPart, secondPart):
        ...
        for i in range(0, len(secondPart)):
            ...
            secondPart = sorted(secondPart,
                key=lambda city: -a*self.distanceBetweenPoints(city.coords(), center) + b*self.distanceBetweenPoints(firstPart[actualListSize - 1].coords(), city.coords()))
...
def ga_solve(file=None, gui=True, maxtime=0, cities = list()):
    ...
    # Generate the initial population with a size of 50 from the problem
